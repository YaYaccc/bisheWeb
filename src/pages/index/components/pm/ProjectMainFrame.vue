        ...
        <Header class="layout-header top-header">
            <div class="top-nav-bar">
                ...
                <div class="nav-toolbar">
                    &nbsp;
                    <ModuleTabbar v-if="projectUUID!=null" :current="currentModule" :value="moduleList"
                                  @change="showModule"></ModuleTabbar>
                ...
                <div class="nav-opt-bar">
                    <div style="float:right;display:inline-block;">
                        ...
                        <IconButton :transfer="true" @click="showNoteView" icon="md-bookmarks"
                                    :tips="$t('笔记')"></IconButton>


<!--                        <IconButton :transfer="true" @click="showSearch=true" size="20" icon="ios-search"-->
<!--                                    :tips="$t('搜索')"></IconButton>-->
                        <IconButton :transfer="true" @click="showNotice=true;unReadNotificationNum=0"
                                    :badge="unReadNotificationNum>0" icon="md-notifications"
                                    :tips="$t('通知')"></IconButton>
<!--                        <IconButton :transfer="true" style="margin-right:10px" @click="showCalendarView"-->
<!--                                    icon="ios-calendar" :tips="$t('日历')"></IconButton>-->
                        <IconButton style="color:#009CF1" :transfer="true" v-if="company&&company.version==1" size="24"
                                    @click="showInviteDialog" icon="ios-person-add" :tips="$t('邀请成员')"></IconButton>


                        <Dropdown transfer style="text-align:left;" transfer-class-name="user-drop-down" trigger="click"
                                  @on-click="clickAccountMenu">
                            <AvatarImage style="cursor:pointer"
                                         :name="account.name"
                                         type="none" v-model="account.imageId"/>
                            <DropdownMenu slot="list" style="width:250px">
                                <DropdownItem @click.native="switchCompany(item.uuid)" v-for="item in companyList"
                                              :key="item.id">
                                    {{ item.name }}
                                    <Icon v-if="item.uuid==company.uuid" style="float:right" type="md-checkmark"/>
                                </DropdownItem>

                                <!--                                <DropdownItem v-if="company&&company.version==2&&perm('cmdb_view')" name="cmdb" divided>
                                                                    {{$t('CMDB')}}
                                                                </DropdownItem>-->
                                <DropdownItem v-if="perm('version_repository_view')&&!isRepositoryVersionExtension"
                                              name="version_repository">
                                    {{ $t('版本库管理') }}
                                </DropdownItem>
                                <!--                                <DropdownItem v-if="company&&company.version==2&&perm('codegen_view')" name="codegen">
                                                                    {{$t('代码助手')}}
                                                                </DropdownItem>-->

<!--                                <DropdownItem name="workTimeStat" divided> {{ $t('工时统计') }}</DropdownItem>-->
                                <DropdownItem name="accountSetting" divided> {{ $t('账号设置') }}</DropdownItem>
                                <DropdownItem v-if="perm('company_admin_manage')" name="companySetting">
                                    {{ $t('企业设置') }}
                                </DropdownItem>
        ...
        <transition name="slide">
            <ProjectMainFrameNoticePopup @popup-close="showNotice=false"
                                         v-if="showNotice"></ProjectMainFrameNoticePopup>
        </transition>
        <transition name="slidedownslow">
            <ProjectMainFrameSearchPopup @popup-close="showSearch=false"
                                         v-if="showSearch"></ProjectMainFrameSearchPopup>
        </transition>
